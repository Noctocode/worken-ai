...
/* ─── Guardrails tab (Settings → Guardrails) ─────────────────────────── */

export function GuardrailsTab() {
  const { t } = useLanguage();
  // Overview / Templates sub-tab is local state — the page-level `?tab=`
  // query already selects the Settings tab (guardrails), so this must not
  // reuse the URL.
  ...
  return (
    <div className="flex flex-col gap-3 py-3 lg:gap-6 lg:py-6">
      {/* Header — title + Add Guardrail. As a Settings tab this no longer
          rides the appbar action, so it owns its own Add button. */}
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-[18px] font-bold text-text-1 lg:text-[20px]">
          {t("guardrails.title")}
        </h2>
        <Button variant="plusAction" onClick={() => setAddOpen(true)}>
